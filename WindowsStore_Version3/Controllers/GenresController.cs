...
namespace WindowsStore_Version3.Controllers
{
    public class GenresController : Controller
    {
        ...
        public ActionResult Index(string cateName, string name)
        {
            //show table 
            var movies = from n in db.Genres select n;

            //condition search
            if (!(String.IsNullOrEmpty(cateName)))
            {
                movies = from n in db.Genres where n.GenreName.Equals(cateName) select n;
            }
            if (!(String.IsNullOrEmpty(name)))
            {
                movies = from n in db.Genres where n.GenreName.Contains(name) select n;
            }
