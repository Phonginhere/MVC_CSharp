	...
	public class NhanViensController : Controller
	{
		...
		public ActionResult Create([Bind(Include = "Employee_ID,Status,Ten,Email,SDT,NgayTao,NgaySua,DOB,MatKhau,Role_ID")] NhanVien nhanVien)
		{
			...
			if (ModelState.IsValid)
			{
				...
				if (nhanVien.Role_ID == 1)
				{
					ViewBag.root = "Chỉ có duy nhất 1 tài khoản root";
					check++;
				}
				if (check > 0)
				{
		...
		public ActionResult Delete(int? id)
		{
			...
			var check_user = db.NhanViens.FirstOrDefault(c => c.Email == username);
			
			if (id == 1)
			{
				TempData["problem"] = "Không được xóa tài khoản root";
				return RedirectToAction("Index");
			}else if (id == check_user.Employee_ID)
			{
				TempData["problem"] = "Không được xóa tài khoản chính mình";
				return RedirectToAction("Index");
			}
		...
		public ActionResult DeleteConfirmed(int id)
		{
			...
			//xóa thông tin trong bảng question chứa id nhân viên
